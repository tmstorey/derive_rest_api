//! Comprehensive async demo of the JSON Placeholder API client
//!
//! This demo showcases async/await usage with WASM support.
//!
//! Run with:
//! ```bash
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== JSON Placeholder API Async Demo ===\n");
    ...
    let posts = client.list_posts().send_async().await?;
    ...
    let user_posts = client.list_posts().user_id(1).send_async().await?;
    ...
    let post = client.get_post().id(1).send_async().await?;
    ...
    let new_post = client
        ...
        .body("This post was created using the derive_rest_api library with async/await. It makes building type-safe REST clients easy!".to_string())
        .user_id(1)
        .send_async()
        .await?;
    ...
    client.delete_post().id(1).send_async().await?;
    ...
    let users = client.list_users().send_async().await?;
    ...
    let user = client.get_user().id(1).send_async().await?;
    ...
    let comments = client.get_post_comments().post_id(1).send_async().await?;
    ...
    let filtered_comments = client.list_comments().post_id(2).send_async().await?;
    println!("   Found {} comments for post #2", filtered_comments.len());
    println!();

    println!("✅ Async demo completed successfully!");
